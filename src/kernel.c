...
void map_page(uint32_t *table1, uint32_t vaddr, paddr_t paddr, uint32_t flags) {
    ...
    // create the second-level page table if it does not exist
...
void virtio_blk_init(void) {
    ...
    // allocate an area to store processing requests to the device
    blk_req_paddr = alloc_pages(align_up(sizeof(*blk_req), PAGE_SIZE) / PAGE_SIZE);
    blk_req = (struct virtio_blk_req *)blk_req_paddr;
}

// notify the new request to the device
// `desc_index`: index of the top descriptor of the new request
...
void read_write_disk(void *buf, unsigned sector, bool is_write) {
    ...
    // construct descriptors of virtqueue
...
void handle_trap(struct trap_frame *f) {
    ...
    if (scause == SCAUSE_ECALL) {
        handle_syscall(f);
        user_pc += 4; // point to the next instruction after ecall
    }
    else {
